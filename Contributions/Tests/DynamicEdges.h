...
public:
	DynamicEdges()
	{
...

		{
			...
			sd_left.SetAsBox(0.15, 2.7, b2Vec2(-1.45, 2.35), 0.2);
			sd_left.density = 4.0;

			b2PolygonDef sd_right;
			sd_right.SetAsBox(0.15, 2.7, b2Vec2(1.45, 2.35), -0.2);
		...
		
		{
			...
			for (int32_t i = 86; i >= 0; i--) {
				b2Loop1[86 - i].Set(loop1[i*2] + 10.0, loop1[i*2 + 1] + 1.0);
			}
			/*for (int32_t i = 0; i < 87; i++) {
				b2Loop1[i].Set(loop1[i*2] + 10.0, loop1[i*2 + 1] + 1.0);
			}*/
