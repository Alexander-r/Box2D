...
void b2GearJoint::SolveVelocityConstraints(const b2SolverData& data)
{
	...
	double Cdot = b2Dot(m_JvAC, vA - vC) + b2Dot(m_JvBD, vB - vD);
...
b2Vec2 b2GearJoint::GetReactionForce(double inv_dt) const
{
	b2Vec2 P = m_impulse * m_JvAC;
	return inv_dt * P;
}

double b2GearJoint::GetReactionTorque(double inv_dt) const
{
	double L = m_impulse * m_JwA;
	return inv_dt * L;
}

void b2GearJoint::SetRatio(double ratio)
{
	b2Assert(b2IsValid(ratio));
	m_ratio = ratio;
}

double b2GearJoint::GetRatio() const
{
	return m_ratio;
}
