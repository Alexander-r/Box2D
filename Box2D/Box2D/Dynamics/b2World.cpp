...
void b2World::SolveTOI(const b2TimeStep& step)
{
	...
	for (;;)
	{
        ...
        if (minContact == nullptr || 1.0 - 10.0 * b2_epsilon < minAlpha)
		{
...
void b2World::Step(double dt, int32_t velocityIterations, int32_t positionIterations)
{
	b2Timer stepTimer;
...
{
	double RayCastCallback(const b2RayCastInput& input, int32_t proxyId)
	{
	...
	case b2Shape::e_chain:
		{
			...
			for (int32_t i = 1; i < count; ++i)
			{
                ...
                m_debugDraw->DrawSegment(v1, v2, color);
                m_debugDraw->DrawPoint(v2, 4.0, color);
				v1 = v2;
			}
			...
			if (chain->m_hasNextVertex)
			{
                ...
                m_debugDraw->DrawCircle(vn, 0.1, ghostColor);
			}
		}
		break;
	...
	case e_mouseJoint:
        {
            b2Color c;
            c.Set(0.0, 1.0, 0.0);
            m_debugDraw->DrawPoint(p1, 4.0, c);
            m_debugDraw->DrawPoint(p2, 4.0, c);
...
double b2World::GetTreeQuality() const
{
