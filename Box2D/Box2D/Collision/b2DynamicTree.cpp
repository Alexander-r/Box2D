...
void b2DynamicTree::InsertLeaf(int32_t leaf)
{
	...
	while (m_nodes[index].IsLeaf() == false)
	{
		...
		double area = m_nodes[index].aabb.GetPerimeter();
		...
		else
		{
			b2AABB aabb;
			aabb.Combine(leafAABB, m_nodes[child1].aabb);
			double oldArea = m_nodes[child1].aabb.GetPerimeter();
		...
		else
		{
			b2AABB aabb;
			aabb.Combine(leafAABB, m_nodes[child2].aabb);
			double oldArea = m_nodes[child2].aabb.GetPerimeter();
			double newArea = aabb.GetPerimeter();
...
//
double b2DynamicTree::GetAreaRatio() const
{
	if (m_root == b2_nullNode)
	{
		return 0.0;
