...
////   of C library functions used by stb_truetype.
   ...
   typedef unsigned short  stbtt_uint16_t;
   typedef signed   short  stbtt_int16_t;
   ...
   typedef char stbtt__check_size16[sizeof(stbtt_int16_t)==2 ? 1 : -1];
...
//
   ...
   #define stbtt_vertex_type short // can't use stbtt_int16_t because that's not visible in the header file
   typedef struct
   {
...
#define ttFixed(p)    ttLONG(p)
   ...
   #define ttUSHORT(p)   (* (stbtt_uint16_t *) (p))
   #define ttSHORT(p)    (* (stbtt_int16_t *) (p))
   #define ttULONG(p)    (* (stbtt_uint32_t *) (p))
   #define ttLONG(p)     (* (stbtt_int32_t *) (p))

#else

   stbtt_uint16_t ttUSHORT(const stbtt_uint8 *p) { return p[0]*256 + p[1]; }
   stbtt_int16_t ttSHORT(const stbtt_uint8 *p)   { return p[0]*256 + p[1]; }
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   stbtt_uint16_t format = ttUSHORT(data + index_map + 0);
   if (format == 0) { // apple byte encoding
      ...
      stbtt_uint16_t segcount = ttUSHORT(data+index_map+6) >> 1;
      stbtt_uint16_t searchRange = ttUSHORT(data+index_map+8) >> 1;
      stbtt_uint16_t entrySelector = ttUSHORT(data+index_map+10);
      stbtt_uint16_t rangeShift = ttUSHORT(data+index_map+12) >> 1;
      stbtt_uint16_t item, offset, start, end;
      ...
      item = (stbtt_uint16_t) ((search - endCount) >> 1);
      ...
      if (offset == 0)
         return (stbtt_uint16_t) (unicode_codepoint + ttSHORT(data + index_map + 14 + segcount*4 + 2 + 2*item));
...
static void stbtt_setvertex(stbtt_vertex *v, stbtt_uint8 type, stbtt_int32_t x, stbtt_int32_t y, stbtt_int32_t cx, stbtt_int32_t cy)
{
   ...
   v->x = (stbtt_int16_t) x;
   v->y = (stbtt_int16_t) y;
   v->cx = (stbtt_int16_t) cx;
   v->cy = (stbtt_int16_t) cy;
}
...
{
   stbtt_int16_t numberOfContours;
   int g = stbtt__GetGlyfOffset(info, glyph_index);
   if (g < 0) return 1;
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      for (i=0; i < n; ++i) {
         ...
         if (flags & 2) {
            stbtt_int16_t dx = *points++;
            x += (flags & 16) ? dx : -dx; // ???
         } else {
            if (!(flags & 16)) {
               x = x + (stbtt_int16_t) (points[0]*256 + points[1]);
               points += 2;
            }
         }
         vertices[off+i].x = (stbtt_int16_t) x;
      }
      ...
      for (i=0; i < n; ++i) {
         flags = vertices[off+i].type;
         if (flags & 4) {
            stbtt_int16_t dy = *points++;
            y += (flags & 32) ? dy : -dy; // ???
         } else {
            if (!(flags & 32)) {
               y = y + (stbtt_int16_t) (points[0]*256 + points[1]);
               points += 2;
            }
         }
         vertices[off+i].y = (stbtt_int16_t) y;
      }
      ...
      for (i=0; i < n; ++i) {
         flags = vertices[off+i].type;
         x     = (stbtt_int16_t) vertices[off+i].x;
         y     = (stbtt_int16_t) vertices[off+i].y;
...
{
   stbtt_uint16_t numOfLongHorMetrics = ttUSHORT(info->data+info->hhea + 34);
                                ...
                                stbtt_bakedchar *chardata)
{
   ...
   for (i=0; i < num_chars; ++i) {
      ...
      chardata[i].x0 = (stbtt_int16_t) x;
      chardata[i].y0 = (stbtt_int16_t) y;
      chardata[i].x1 = (stbtt_int16_t) (x + gw);
      chardata[i].y1 = (stbtt_int16_t) (y + gh);
...
static stbtt_int32_t stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8 *s1, stbtt_int32_t len1, const stbtt_uint8 *s2, stbtt_int32_t len2) 
{
   ...
   while (len2) {
      stbtt_uint16_t ch = s2[0]*256 + s2[1];
      if (ch < 0x80) {
         if (i >= len1) return -1;
         if (s1[i++] != ch) return -1;
      } else if (ch < 0x800) {
         ...
         stbtt_uint32_t c;
         stbtt_uint16_t ch2 = s2[2]*256 + s2[3];
